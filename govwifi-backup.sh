...
MYSQL_PWD="${WIFI_DB_PASS}" mysqldump -h "${WIFI_DB_HOST}" -u "${WIFI_DB_USER}" \
  --compress --quick --single-transaction --no-create-info --complete-insert "${WIFI_DB_NAME}" \
  | gzip -c | openssl enc -base64 -pass pass:${ENCRYPTION_KEY} | aws ${ENDPOINT_ARG} s3 cp - s3://"${S3_BUCKET}/wifi-backup-$(date -I)".sql.gz.enc

STATUS1=$?
[ $STATUS1 -eq 0 ] && echo COMPLETE || echo FAILED

echo -n "STARTING SQL DUMP OF USERS DB - "
MYSQL_PWD="${USERS_DB_PASS}" mysqldump -h "${USERS_DB_HOST}" -u "${USERS_DB_USER}" \
  --compress --quick --single-transaction --no-create-info --complete-insert "${USERS_DB_NAME}" \
  | gzip -c | openssl enc -base64 -pass pass:${ENCRYPTION_KEY} | aws ${ENDPOINT_ARG} s3 cp - s3://"${S3_BUCKET}/wifi-backup-user-details-$(date -I)".sql.gz
STATUS2=$?
[ $STATUS2 -eq 0 ] && echo COMPLETE || echo FAILED

echo -n "STARTING SQL DUMP OF ADMIN DB - "
MYSQL_PWD="${ADMIN_DB_PASS}" mysqldump -h "${ADMIN_DB_HOST}" -u "${ADMIN_DB_USER}" \
  --compress --quick --single-transaction --no-create-info --complete-insert "${ADMIN_DB_NAME}" \
  | gzip -c | openssl enc -base64 -pass pass:${ENCRYPTION_KEY} | aws ${ENDPOINT_ARG} s3 cp - s3://"${S3_BUCKET}/wifi-backup-admin-$(date -I)".sql.gz
STATUS3=$?
